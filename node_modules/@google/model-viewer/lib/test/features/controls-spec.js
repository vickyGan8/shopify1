    ...
    suite('camera-controls', () => {
        ...
        suite('when user is interacting', () => {
            ...
            test('does not send "user-interaction" after JS change', async () => {
                const user = waitForEvent(element, 'camera-change', (event) => event.detail.source === ChangeSource.USER_INTERACTION);
                dispatchSyntheticEvent(element[$userInputElement], 'keydown', { key: 'ArrowUp' });
                await user;
                const js = waitForEvent(element, 'camera-change', (event) => event.detail.source === ChangeSource.NONE);
        ...
        suite('a11y', () => {
            ...
            test('announces camera orientation when orbiting horizontally with a11y', async () => {
                await rafPasses();
                input.focus();
                element.a11y = {
            ...
            test('announces camera orientation when orbiting vertically with a11y', async () => {
                await rafPasses();
                input.focus();
                element.a11y = {
                    'front': 'A11y test stage front',
